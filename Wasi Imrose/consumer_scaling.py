...
TOPIC = "topic_2"    # new topic
OUTPUT_CSV = "scaling_metrics.csv"
...
class MetricsTracker:
    ...
    def record(self, produce_ts, consume_ts):
...
# Consumer Loop - This was created using a mix of Confluent and Python documentation and Clause AI for debugging

def run_consumer():
    config = read_config()
    config.update({
        "group.id": "hft-scaling-group",   # ✅ separate group
    ...
    print("Waiting for scaling producer. Start scaling_producer.py now.\n")
    ...
    try:
        ...
        print(f"Avg latency:         {np.mean(tracker.latencies):.1f}ms")
        p95_final = float(np.percentile(tracker.latencies, 95)) if tracker.latencies else 0
        p99_final = float(np.percentile(tracker.latencies, 99)) if tracker.latencies else 0
        print(f"P95 latency:         {p95_final:.1f}ms")
        print(f"P99 latency:         {p99_final:.1f}ms")
    finally:
        consumer.close()
        print("Done. CSV saved to scaling_metrics.csv")
