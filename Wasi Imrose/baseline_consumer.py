...
def seek_to_end(consumer, topic):
    # manually assign and seek to end of all partitions
...
def run_consumer():
    config = read_config()
    config.update({
        "group.id": "hft-metrics-group",
        "enable.auto.commit": True
    })

    consumer = Consumer(config)

    # seek to end before subscribing — skips all stale messages
    seek_to_end(consumer, TOPIC)

    init_csv()
    tracker = MetricsTracker()

    print(f"Consuming from {TOPIC}... writing metrics to {OUTPUT_CSV}")
    print("Waiting for new messages. Start your producer now.\n")
        ...
        with open(OUTPUT_CSV, "a", newline="") as f:
            ...
            while True:
                ...
                if "id" not in data or "timestamp" not in data:
                    print(f"Skipping non-HFT message: {data}")
                    continue

                produce_ts = data["timestamp"]
                latency_ms, throughput, p95, p99 = tracker.record(produce_ts, consume_ts)
                ...
                if tracker.msg_count % 20 == 0:
                    print(f"Msg {tracker.msg_count} | "
